...
-----------------------------------------------*/
ENT.Model = "" 
ENT.StartHealth = GetConVarNumber("vj_tanz_h")
...
ENT.NextAnyAttackTime_Melee = 0.75
...
ENT.MeleeAttackDamage = GetConVarNumber("vj_tanz_d")
ENT.MeleeAttackDamageType = DMG_SLASH -- Type of Damage
ENT.TimeUntilMeleeAttackDamage = 0.65
ENT.MeleeAttackReps = 1
ENT.MeleeAttackExtraTimers = {1.15 }
...
function ENT:CustomInitialize()
	self:SetCollisionBounds(Vector(25, 18, 80), Vector(-15, -18, 0))
...
function ENT:CustomOnInitialize()
	...
	if randomstartskin == 1 then self:SetModel( Model("models/tan.mdl") ) else
	if randomstartskin == 2 then self:SetModel( Model("models/PCZOMBIE.mdl") ) end
end
