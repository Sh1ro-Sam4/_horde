...
SWEP.Purpose				= "Multirole weapon. Right click to change to rifle. Using the scope in rifle mode turns in into sniper mode, consumes 5 ammo per shot. Suitzoom button reverts back to smg mode" --Purpose Tooltip
...
SWEP.Primary.Damage_Secondary		= 90					-- Damage, in standard damage points.
...
function SWEP:Think2(...)
	...
	elseif self:GetIronSightsProgress() > 0.5 and not self:GetCSO_SwitchState() then
		    self.TracerName = "tra_xtracker_sniper"
			self.Primary_TFA.Damage = self.Primary_TFA.Damage_Secondary * 8
			...
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "muz_xtracker"
			self.Primary_TFA.KickUp	= 0.175	
            self.Primary_TFA.KickDown	= 0.17		
