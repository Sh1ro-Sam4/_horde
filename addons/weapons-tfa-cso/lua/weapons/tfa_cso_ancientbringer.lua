...
SWEP.PrintName				= "Ancient Bringer"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.Damage_Secondary		= 275					-- Damage, in standard damage points.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_ancientbringer.mdl" --Viewmodel path
...
--	["bone_up02"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }
--}

--[[WORLDMODEL]]--

SWEP.WorldModel			= "models/weapons/tfa_cso/w_ancient_bringer.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_ancientbringer" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
SWEP.MuzzleFlashEffectSilenced = "cso_muz_ancientbringer"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_ancientbringer"	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Primary.ImpactEffect = "exp_ancientbringer"
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_ancientbringer")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.MuzzleFlashEffect = "cso_muz_ancientbringer"
			self.Primary_TFA.ImpactEffect = nil
			self.TracerName = "cso_tra_ancientbringer"
			self:ClearStatCache()
		else
			self.TracerName = nil
			...
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "cso_muz_ancientbringer"
			self.Primary_TFA.ImpactEffect = "exp_ancientbringer"
