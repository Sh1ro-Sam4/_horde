...
if(SERVER) then
	hook.Add("PlayerSwitchWeapon", "CSO_MountGun_NoSwitch", function( player, oldWeapon, newWeapon )
		if(!IsValid(oldWeapon)) then return end
		if(oldWeapon:GetClass() == "tfa_cso_mountedgun") then return true end
	end)

	hook.Add("PlayerUse", "CSO_MountedGun_Use", function(ply, ent)
		if(!IsValid(ply) || !IsValid(ent)) then return end
		if(ply.CSO_UseCD != nil && ply.CSO_UseCD > CurTime()) then return end
		ply.CSO_UseCD = CurTime() + 1
		if(!ply:Alive()) then return end
		if(ent:GetClass() == "cso_mounted_gun") then
			if(ent.CurrentUser == nil) then
				ent.CurrentUser = ply
				ent.LastOwner = ply
				ply:SetVelocity(ply:GetVelocity() * -1)
				ply:SetPos(ent:GetPos() + Angle(0, ent:GetAngles().y, 0):Forward() * -30)
				ply:SetEyeAngles(ent:GetAngles())
				ply:SetNWEntity("CSO_MountGun", ent)
			else
				if(ent.CurrentUser == ply) then
					ent.CurrentUser = nil
				end
			end
		end
...
-- if(CLIENT) then
-- 	hook.Add( "Tick", "CSO_HideMountGun",  function()
-- 		if(IsValid(LocalPlayer():GetNWEntity("CSO_MountGun"))) then
-- 			LocalPlayer():GetNWEntity("CSO_MountGun"):SetNoDraw(true)
-- 			LocalPlayer().LastEntityNoDraw = LocalPlayer():GetNWEntity("CSO_MountGun")
-- 		else
-- 			if(IsValid(LocalPlayer().LastEntityNoDraw)) then
-- 				LocalPlayer().LastEntityNoDraw:SetNoDraw(false)
-- 			end
-- 		end
-- 	end)
-- end
...
function SWEP:PostPrimaryAttack()
	if(self:Clip1() <= 0) then self:EmitSound(self.DrySound) end
	if(SERVER) then
		for k,v in pairs(ents.FindByClass("cso_mounted_gun")) do
			if(self:GetOwner() == v.CurrentUser) then
				v.CurrentAmmo = math.Clamp(v.CurrentAmmo - 1, 0, 250)
				v.TimeToRegen = CurTime() + 1.5
			end
		end
	end
end