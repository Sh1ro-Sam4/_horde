...
SWEP.PrintName				= "Cyclops"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("Cyclops.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_cyclops.mdl" --Viewmodel path
...
SWEP.ViewModelBoneMods = {
	["wing_R_1"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-45, 0, 0) },
	["wing_R_4"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(35, 0, 0) },
	["wing_L_1"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-45, 0, 0) },
	["wing_L_4"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(35, 0, 0) }
}

--[[WORLDMODEL]]--

SWEP.WorldModel			= "models/weapons/tfa_cso/w_cyclops.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_cyclops" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "tra_cyclops" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Primary.ImpactEffect = "exp_cyclops_a"
...
SWEP.LaserAnimation = {
	...
	["loop"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
		["value"] = "shoot2_charging1", --Number for act, String/Number for sequence
		["is_idle"] = true
	},--looping animation
	["end"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
...
SWEP.LaserSound_Loop = "Cyclops.Fire2"
SWEP.LaserSound_End = "Cyclops.Fire2_End"
...
SWEP.LaserDrain_Loop = 220 / 10--Ammo per second

SWEP.Secondary.Damage = 120
SWEP.Secondary.MaxAmmo = 100
SWEP.Secondary.Reach = 800
...
SWEP.Secondary.Ammo			= "energy_cell" -- Self explanitory, ammo type.
		...
		if game.GetTimeScale() > 0.99 then
			self.Owner:FireBullets({
				...
				Callback = function(a, b, c)
					c:SetDamageType( bit.bor( DMG_DISSOLVE,DMG_AIRBOAT,DMG_ALWAYSGIB ) )
				end
			})
		else
			...
			dmg:SetDamage(120)
			dmg:SetDamageType( bit.bor( DMG_DISSOLVE,DMG_AIRBOAT,DMG_ALWAYSGIB ) )
...
function SWEP:PostPrimaryAttack()
   ...
   if SERVER then
      if self.NextGen >= 1 and self:Ammo2() < 100 then
...
function SWEP:Think()
   if SERVER then
      if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 100 then
         self.Owner:GiveAmmo( 4,self.Secondary.Ammo, true )                     -------SECONDARY AMMO REGEN
...
function SWEP:Holster( ... )
   if self:Ammo2() > 10 and TFA.Enum.ReadyStatus[ self:GetStatus() ] then
      self:TakeSecondaryAmmo(100)
      local effectdata = EffectData()
      effectdata:SetEntity(self.Owner)
      effectdata:SetOrigin(self.Owner:GetPos())
      effectdata:SetAngles(Angle(0,0,0))
      util.Effect("exp_cyclops", effectdata)
      local radius = 250
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_cyclops")
