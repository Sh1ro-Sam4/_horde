...
SWEP.PrintName				= "Bear Fury MK-3"		-- Weapon name (Shown on HUD)	
...
SWEP.Primary.Damage		= (engine.ActiveGamemode() == "sandbox" and 30) or 60					-- Damage, in standard damage points.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_bearfurymk3.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_bearfurymk3_right.mdl" -- Worldmodel path
...
SWEP.WElements = {
    ["bearfurymk3_l"] = { type = "Model", model = "models/weapons/tfa_cso/w_bearfurymk3_left.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(0, -1, 0), angle = Angle(0, 85, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
    ...
    ["jetpack"] = { type = "Model", model = "models/weapons/tfa_cso/w_bearfurymk3_jetpack.mdl", bone = "ValveBiped.Bip01_Spine2", rel = "", pos = Vector(5, 7, 0), angle = Angle(90, 90, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Secondary.Sound = Sound("BearFuryMK3.Dash")
SWEP.Secondary.ClipSize			= -1					-- Size of a clip
SWEP.Secondary.DefaultClip			= 0					-- Default ammo to give...
SWEP.Secondary.Automatic			= false					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "bearjumpmk3" -- Self explanitory, ammo type.
...
function SWEP:PostPrimaryAttack()
	...
	if SERVER then
		if self.NextGen >= 60 and self:Ammo2() < 2 then
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("BearFuryMK3.Fire")
			self.NextIdleSound = CurTime() + 0
		else
			self:EmitSound("BearFuryMK3.Fire_End")
	...
	if shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("BearFuryMK3.Fire")
			self.NextIdleSound = CurTime() + SoundDuration( "BearFuryMK3.Fire" ) - 0.1
		end
	end
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_bearfurymk3")
