...
SWEP.PrintName				= "Lightning Fury"		-- Weapon name (Shown on HUD)	
...
SWEP.Primary.Damage		= (engine.ActiveGamemode() == "sandbox" and 32) or 64					-- Damage, in standard damage points.
...
--Range Related
SWEP.Primary.Range = 550 -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_jetgun.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_jetright.mdl" -- Worldmodel path
...
SWEP.WElements = {
    ["jetgun_l"] = { type = "Model", model = "models/weapons/tfa_cso/w_jetleft.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(0, 1, 1), angle = Angle(0, 80, 0), size = Vector(1.4, 1.4, 1.4), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
    ...
    ["jetpack"] = { type = "Model", model = "models/weapons/tfa_cso/w_jetpack.mdl", bone = "ValveBiped.Bip01_Spine2", rel = "", pos = Vector(5, 7, 0), angle = Angle(90, 90, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
} 
...
SWEP.EventTable = {
    [ACT_VM_DRAW] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            ...
            wep.WElements.tracer_l.active = false
            wep.WElements.tracer_r.active = false
            wep:ClearStatCache("VElements.addictive01.active")
            wep:ClearStatCache("VElements.addictive02.active")
            wep:ClearStatCache("WElements.tracer_l.active")
            wep:ClearStatCache("WElements.tracer_r.active")
        end}
    },
    [ACT_VM_IDLE] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            ...
            wep:ClearStatCache("WElements.tracer_l.active")
            wep:ClearStatCache("WElements.tracer_r.active")
            wep:ClearStatCache("WElements.flame_l.active")
            wep:ClearStatCache("WElements.flame_r.active")
        end}
    },
    [ACT_VM_RELOAD] = {
    ...
    [ACT_VM_PRIMARYATTACK] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            wep.VElements.addictive01.active = false
            wep.VElements.addictive02.active = false
            wep.WElements.tracer_l.active = true
            wep.WElements.tracer_r.active = true
            wep:ClearStatCache("VElements.addictive01.active")
            wep:ClearStatCache("VElements.addictive02.active")
            wep:ClearStatCache("WElements.tracer_l.active")
            wep:ClearStatCache("WElements.tracer_r.active")
...
SWEP.Secondary.Ammo			= "jetpack" -- Self explanitory, ammo type.
...
function SWEP:SecondaryAttack()
	if self:Ammo2() > 0 and TFA.Enum.ReadyStatus[ self:GetStatus() ] and self.Owner:OnGround() then
		self.Owner:SetVelocity(self.Owner:GetForward() * 600 + self.Owner:GetUp() * 300)
...
function SWEP:PostPrimaryAttack()
	...
	if SERVER then
		if self.NextGen >= 30 and self:Ammo2() < 2 then
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_jetgun")
