...
SWEP.PrintName				= "Ancient Keeper"		-- Weapon name (Shown on HUD)
...
SWEP.NZPaPName				= "The Thunder Prison"
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_ancientkeeper.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_ancient_keeper.mdl" -- Worldmodel path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_ancientkeeper")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.Damage = 84
			self.Primary_TFA.DamageType = bit.bor (DMG_SHOCK,DMG_DISSOLVE,DMG_AIRBOAT)
			...
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "cso_muz_ancientkeeperb"
			self.TracerName = "cso_tra_ancientkeeperb"
		    self:ClearStatCache()
		end
	end
	...
	if self:GetSilenced() and shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("AncientKeeper.Fire2")
			self.NextIdleSound = CurTime() + 0
		else
			self:StopSound("AncientKeeper.Fire2")
			self.NextIdleSound = -1
		end
	end
	if self:GetSilenced() and shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("AncientKeeper.Fire2")
			self.NextIdleSound = CurTime() + SoundDuration( "AncientKeeper.Fire2" ) - 0.1
		end
	end
...
SWEP.Secondary.Ammo			= "keeper_alt" -- Self explanitory, ammo type.
...
self:EmitSound("AncientKeeper.Ready")
end
