...
SWEP.PrintName				= "Doom Blaster"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Doom_Blaster.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("Doom_Blaster.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 84					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular					-- Damage, in standard damage points.
SWEP.Primary.Damage_Secondary		= 12000					-- Damage, in standard damage points.
SWEP.Primary.BlastRadius	= 175               -- Radius for DMG_BLAST
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_doom_blaster.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_doom_blaster.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_winggunex" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
SWEP.MuzzleFlashEffectSilenced = "cso_muz_winggunex"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_winggunex"	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_doom_blaster")
	SWEP.DrawWeaponSelection = TFA_CSO_DrawWeaponSelection
end

DEFINE_BASECLASS( SWEP.Base )
function SWEP:Holster( ... )
	self:StopSound("Doom_Blaster.Loop")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.TracerName = "cso_tra_winggunex"
			...
			self.Primary_TFA.ImpactEffect = "exp_winggunex"
			self:ClearStatCache()
end
end
