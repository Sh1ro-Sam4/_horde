...
if self:Clip1() <= 11 then return end
   ...
   if self.combo < self.comboright + self.comboleft then
      self.combo = self.combo + 1
      util.BlastDamage( self.Owner, self.Owner,self.Owner:GetPos(),350, 1500 )  --damage
      ...
      util.Effect( "exp_harmonium", effectdata )  --
   elseif self.combo >= self.comboright + self.comboleft then
      ...
      util.BlastDamage( self.Owner, self.Owner,self.Owner:GetPos(),375, 1500 )  --damage
      for k, v in pairs(ents.FindInSphere(self:GetPos(),375)) do
         if IsValid( v ) then
            --if v == self then continue end
            --if v == self.Owner then continue end
...
hook.Add( "PlayerShouldTakeDamage", "antiselfdamage2", function( ply, attacker )
if ply == attacker and (ply:GetActiveWeapon():GetClass() == "tfa_cso_harmonium"  or  ply:GetActiveWeapon():GetClass() == "tfa_cso_pianogunex") then
return false
end
end )