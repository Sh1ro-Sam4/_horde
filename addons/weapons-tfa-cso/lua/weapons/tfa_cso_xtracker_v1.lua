...
SWEP.PrintName				= "X-TRACKER (Proto)"		-- Weapon name (Shown on HUD)
...
SWEP.Secondary.Ammo = "xtracker_missile"
SWEP.Secondary.Ent = "xtracker"
SWEP.Secondary.Automatic = true
SWEP.Secondary.Model = "models/hunter/blocks/cube025x025x025.mdl" --Projectile Model
SWEP.Secondary.Damage = 100 --Grenade Damage
SWEP.Secondary.Velocity = 2500 --Grenade Velocity
SWEP.Secondary.Delay = 1 --Delay in seconds
SWEP.Secondary.Sound = Sound("XTracker.ShootB") --Firing sound
...
function SWEP:AltAttack()
	...
	if self:GetSilenced() and self:Ammo2() < 10 then return end
	if self:GetSilenced() and CurTime() > self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Ammo2() == 10 then
		self:TakeSecondaryAmmo(10)
		self:GetOwner():SetAnimation(PLAYER_ATTACK1)
	timer.Simple(1, function()
		for i=10,1,-1 do  --i = 6 - 6 shoots
		if SERVER then
...
if SERVER then
if self.NextGen >= 1 and self:Ammo2() < 10 and self:GetCSO_SwitchState() then
self.Owner:GiveAmmo( 1,self.Secondary.Ammo, true )
self:EmitSound("XTracker.Beep")          
self.NextGen  = 0
end
