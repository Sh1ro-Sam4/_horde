...
SWEP.PrintName				= "Pierrot Magic Bow"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("MagicBow.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.Damage_Secondary		= 768				-- Damage, in standard damage points.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_magicbow.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_magicbow.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_magicbow" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

SWEP.ImpactEffect = "tfa_metal_impact"--Impact Effect
SWEP.ImpactDecal = nil--Impact Decal
SWEP.Primary.ImpactEffect = "exp_magicbow"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_magicbow" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_magicbow")
...
function SWEP:Think2(...)
	if self:GetIronSightsProgress() <= 0.5 and self:GetCSO_SwitchState() then
		...
		self.Primary_TFA.Sound = Sound("MagicBow.Fire")
		self.TracerName = "cso_tra_magicbow"
		...
		self.MuzzleFlashEffect = "cso_muz_magicbow"
		self.Primary_TFA.ImpactEffect = "exp_magicbow"
	...
	elseif self:GetIronSightsProgress() > 0.5 and not self:GetCSO_SwitchState() then
		...
		self.Primary_TFA.Sound = Sound("MagicBow.Fire2")
