...
SWEP.PrintName				= "Pierrot Chameleon Gun"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("ChameleonGun.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_chameleongun.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_chameleongun.mdl" -- same case UHGI;DFSD/
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_chameleongun")
	SWEP.DrawWeaponSelection = TFA_CSO_DrawWeaponSelection
end

DEFINE_BASECLASS(SWEP.Base)
function SWEP:Holster( ... )
	self:StopSound("ChameleonGun.Idle")
	return BaseClass.Holster(self,...)
end

SWEP.Secondary.ClipSize			= -1					-- Size of a clip
SWEP.Secondary.DefaultClip			= 60					-- Default ammo to give...
SWEP.Secondary.Automatic			= true					-- Automatic/Semi Auto
SWEP.Secondary.Ammo			= "chameleon_stealth" -- Self explanitory, ammo type.
...
function SWEP:Think(...) 
	...
	if SERVER then
		...
		self:TakeSecondaryAmmo( 1,"chameleon_stealth", true )             -- Secondary ammo consume
		self.Secondary.AmmoConsume =  CurTime()  + 0.1   -- Seconds to consume
		self.Secondary.Nextregen =  CurTime()  + 6   ---6 seconds
else
		if self.Secondary.Nextregen <= CurTime() and self:Ammo2() < 60 and self:GetNWBool("W_CloakOn") == false then 
		self.Owner:GiveAmmo( 10,"chameleon_stealth", true )                     -------SECONDARY AMMO REGEN
		self.Secondary.Nextregen =  CurTime()  + 6   ---6 seconds
end
	end
end	
