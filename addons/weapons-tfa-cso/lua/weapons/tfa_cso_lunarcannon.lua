...
SWEP.PrintName				= "Lunar Cannon"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("LunarCannon.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Secondary.Ammo = "moon"
SWEP.Secondary.Ent = "moon"

SWEP.Secondary.Model = "models/hunter/misc/sphere1x1.mdl" --Projectile Model
SWEP.Secondary.Damage = 10000 --Grenade Damage
SWEP.Secondary.Velocity = 250 --Grenade Velocity
SWEP.Secondary.Delay = 1 --Delay in seconds
SWEP.Secondary.Sound = Sound("LunarCannon.Fire2") --Firing sound
...
SWEP.NZPaPName				= "End of the World"
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_lunarcannon.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_lunarcannon.mdl" -- same case UHGI;DFSD/
...
function SWEP:Think()
    if SERVER then
        if self.NextGen <= CurTime() and self:Ammo2() < 1 then
        end
    end
...
function SWEP:PostPrimaryAttack()
    if SERVER then
        ...
        if self.NextGen >= 45 and self:Ammo2() < 1 then
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_lunarcannon")
