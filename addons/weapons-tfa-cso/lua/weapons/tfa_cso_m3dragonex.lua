local soundData = {
	...
	pitchstart 	= 100,
	pitchend 	= 100,
	sound 		= "weapons/tfa_cso/m3dragon/reload_insert.wav"
}
...
local soundData = {
	...
	pitchstart 	= 100,
	pitchend 	= 100,
	sound 		= "weapons/tfa_cso/m3dragon/reload_finish.wav"
}

sound.Add(soundData)

TFA.AddFireSound( "M3Dragonex.Shoot", "weapons/tfa_cso/m3dragonex/fire.wav", false, "^" )
TFA.AddFireSound( "M3Dragonex.Shoot2", "weapons/tfa_cso/m3dragonex/fire2.wav", false, "^" )
TFA.AddFireSound( "M3Dragonex.Shoot3", "weapons/tfa_cso/m3dragonex/fire3.wav", false, "^" )
TFA.AddWeaponSound( "M3Dragonex.Idle", "weapons/tfa_cso/m3dragonex/idle.wav" )
TFA.AddWeaponSound( "M3Dragonex.Idle2", "weapons/tfa_cso/m3dragonex/idle2.wav" )
TFA.AddWeaponSound( "M3Dragonex.Charge", "weapons/tfa_cso/m3dragonex/charge.wav" )
TFA.AddWeaponSound( "M3Dragonex.Charge_Attack", "weapons/tfa_cso/m3dragonex/charge_attack.wav" )
TFA.AddWeaponSound( "M3Dragonex.Charge_Dragon", "weapons/tfa_cso/m3dragonex/charge_dragon.wav" )
TFA.AddWeaponSound( "M3Dragonex.Dragon", "weapons/tfa_cso/m3dragonex/dragon.wav" )
...
SWEP.PrintName				= "M3 Azhi Dahaka"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("M3Dragonex.Shoot")	-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.RangeFalloffLUT = {
    bezier = false, -- Whenever to use Bezier or not to interpolate points? set true to straight damage fall after reaching the range
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_m3dragonex.mdl" --Viewmodel path
...
SWEP.EventTable = {
    [ACT_VM_RELOAD] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            wep.VElements.flame_right.active = false
			wep.VElements.flame_left.active = false
            wep:ClearStatCache("VElements.flame_right.active")
			wep:ClearStatCache("VElements.flame_left.active")
        end}
    },
    ...
    [ACT_VM_IDLE] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            wep.VElements.flame_right.active = true
			wep.VElements.flame_left.active = true
            wep:ClearStatCache("VElements.flame_right.active")
			wep:ClearStatCache("VElements.flame_left.active")
        end}
    },
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_m3dragonex.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_m3dragon_ex" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
SWEP.MuzzleFlashEffectSilenced = "cso_muz_m3dragon_ex"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_m3dragon_ex" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_m3dragonex")
	SWEP.DrawWeaponSelection = TFA_CSO_DrawWeaponSelection
end

DEFINE_BASECLASS(SWEP.Base)
function SWEP:Holster( ... )
	self:StopSound("M3Dragonex.Idle")
	self:StopSound("M3Dragonex.Idle2")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.Projectile = "m3dragonex_bomb"
			self.Primary_TFA.ProjectileVelocity = 5000
