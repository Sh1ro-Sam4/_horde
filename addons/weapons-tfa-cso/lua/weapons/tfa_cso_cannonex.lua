...
SWEP.UntransTime = 0
SWEP.AllowedtoDamage = false
SWEP.TimeToSummon = 0
SWEP.DragonSummoned = false
...
SWEP.BaseBlastDamage = (engine.ActiveGamemode() == "sandbox" and 6000) or 12500
...
function SWEP:Think()
	...
	if(seq == 6 && dur >= 3.2) then
		if(self.AllowedtoDamage) then
	...
	if(seq == 7 && dur >= 2.5) then
		owner:EmitSound( "weapons/tfa_cso/cannonex/cannonex_d_reload2.wav" )
		self:ChooseSilenceAnim( not self:GetSilenced() )
		self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
		self:SetStatusEnd(CurTime() + self:GetActivityLength( tanim ) - 0.1)
		self:SetNextPrimaryFire(CurTime() + self:GetActivityLength( tanim ) - 0.3)
		self.DragonSummoned = false
	end
	if(self:GetSilenced()) then
		self.ProjectileDamage = self.BaseBlastDamage * 2
	else
		self.ProjectileDamage = self.BaseBlastDamage
	end
end
...
function SWEP:SecondaryAttack()
	...
	if(CLIENT) then
		if(!self:GetSilenced()) then
			sound.PlayFile( 'sound/weapons/tfa_cso/cannonex/cannonex_dtransform.wav', 'noplay', function( station, errCode, errStr ) station:Play() end)
		else
			sound.PlayFile( 'sound/weapons/tfa_cso/cannonex/cannonex_d_reload2.wav', 'noplay', function( station, errCode, errStr ) station:Play() end)
		end
	end
	...
	if SERVER or not sp then
		self:ChooseSilenceAnim( not self:GetSilenced() )
		self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
		self:SetStatusEnd(CurTime() + self:GetActivityLength( tanim ) - 0.2)
		self:SetNextPrimaryFire(CurTime() + self:GetActivityLength( tanim ) - 0.2)
		self.TimeToSummon = CurTime() + 3.5
		self.UntransTime = CurTime() + 10
		self.AllowedtoDamage = true
		return
	end
end
...
function SWEP:PostPrimaryAttack()
	...
	local dur = vm:SequenceDuration(seq) * cyc
	if(self:GetSilenced()) then
		if(self.UntransTime < CurTime()) then
			self:GetOwner():GetViewModel(0):SetSequence(7)
			self:SetStatusEnd(CurTime() + 3.5)
		end
	end
	if(SERVER) then
		if(self.TimeToSummon >= CurTime() && !self.DragonSummoned) then
			local dragon_ = ents.Create("cannonex_dragon")
				dragon_:SetOwner(owner)
				dragon_:Spawn()
			self.DragonSummoned = true
		end
	end
end