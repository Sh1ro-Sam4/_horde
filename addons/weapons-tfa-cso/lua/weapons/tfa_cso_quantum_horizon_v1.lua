...
SWEP.PrintName				= "Quantum Horizon (Proto)"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Quantum.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.Damage		= 1500					-- Damage, in standard damage points.
SWEP.Primary.BlastRadius	= 125               -- Radius for DMG_BLAST
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_quantum_horizon.mdl" --Viewmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_quantum" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
SWEP.Primary.ImpactEffect = "exp_quantum"
--[[EVENT TABLE]]--
...
SWEP.Secondary.Sound 			= Sound("Quantum.Fire2")
SWEP.Secondary.Damage		= 463
...
function SWEP:Think() 
if SERVER then
if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 6 then 
self.Owner:GiveAmmo( 1,"slam", true )                     -------SECONDARY AMMO REGEN
...
local eye = self:GetOwner():GetEyeTrace()
	timer.Simple( 0.25, function() 
		pcall(function()
			self:GetOwner():SetPos(eye.HitPos)
			self:EmitSound("QuantumHorizon.Revive_Out", 256)
		end)
	end )
	end
self:SetNextSecondaryFire( CurTime()  + 25 )
	if SERVER or not sp then
		...
		self:SetStatusEnd(CurTime() + 0)
		self:EmitSound("QuantumHorizon.Revive_In", 256)
		return
	end
end
...
bullet.TracerName = "cso_tra_revivegun"
