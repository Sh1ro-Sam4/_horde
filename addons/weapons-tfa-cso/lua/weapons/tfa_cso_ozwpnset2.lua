...
SWEP.PrintName				= "Oz Lion Pistol"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("OZWPNSET2.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_ozwpnset2.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_ozwpnset2.mdl" -- Worldmodel path
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_ozwpnset2")
...
function SWEP:SecondaryAttack( ... )
        if self:Clip1() > 0 and not self:GetSprinting() and TFA.Enum.ReadyStatus[ self:GetStatus() ] then
			...
			self.Primary_TFA.Damage = 24
            self.Primary_TFA.RPM_Semi = 800
        ...
        elseif self:GetOwner():KeyDown(IN_ATTACK2) then
		    self.Primary_TFA.Damage = 27
            self.Primary_TFA.RPM_Semi = 500
            self.Primary_TFA.KickUp    = 0.075
...
function SWEP:Holster( ... )
	self:StopSound("OZWPNSET2.Idle1")
	self:StopSound("OZWPNSET2.Idle2")
	self:StopSound("OZWPNSET2.Idle3")
