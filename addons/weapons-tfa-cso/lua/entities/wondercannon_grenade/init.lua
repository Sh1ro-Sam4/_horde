...
function ENT:Initialize()
	self:SetModel("models/weapons/tfa_cso/w_wondercannonex_bomb.mdl")
...
function ENT:Think()
	if(!self.Set) then
		...
		if(tr_.HitWorld) then
			sound.Play("weapons/tfa_cso/heaven_scorcher/mine_set.wav", self:GetPos() + Vector(0, 0, 6), 100, 100, 1)
			self.Set = true
			self:SetAngles(Angle(0, 0, 0))
			self:GetPhysicsObject():EnableMotion(false)
			self:SetPos(tr_.HitPos)
			self.LightSprite = ents.Create("env_sprite_oriented")
			self.LightSprite:SetKeyValue("model","materials/sprites/ef_wondercannonex_mine_set.vmt")
			self.LightSprite:SetKeyValue("scale", "0.125")
			self.LightSprite:SetPos(self:GetPos() + Vector(0, 0, 12))
...
function ENT:OnRemove()
	...
	self.EXPSprite:SetKeyValue("model","materials/sprites/ef_wondercannonex_mine_exp.vmt")
	...
	sound.Play("weapons/tfa_cso/wondercannonex/exp.wav", self:GetPos() + Vector(0, 0, 5), 100, 100, 1)
