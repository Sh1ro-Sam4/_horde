...
function EFFECT:Init(data)
	...
	local sparticle = emitter:Add("sprites/ef_crossbowex21_muzzleflash2", self.vOffset)

	if (sparticle) then
		sparticle:SetVelocity(dir * 4 + 1.05 * AddVel)
		sparticle:SetLifeTime(0)
		sparticle:SetDieTime(0.30)
		sparticle:SetStartAlpha(math.Rand(255, 255))
		sparticle:SetEndAlpha(0)
		--particle:SetStartSize( 8 * (halofac*0.8+0.2), 0, 1)
		--particle:SetEndSize( 0 )
		sparticle:SetStartSize(25 * (halofac * 0.8 + 0.2), 0, 1)
		sparticle:SetEndSize(20 * (halofac * 0.8 + 0.2), 0, 1)
