CreateConVar("sv_tfa_cso_budgetsword_skill_damage", 65, FCVAR_ARCHIVE, "Dual Sword Hellfire's special combo damage, per tick.", 1, 32767)
CreateConVar("sv_tfa_cso_dualsword_skill_damage", 70, FCVAR_ARCHIVE, "Dual Sword Phantom Slayer's special combo damage, per tick.", 1, 32767)
CreateConVar("sv_tfa_cso_dualsword_rb_skill_damage", 80, FCVAR_ARCHIVE, "Dual Sword Rainbow Caster's special combo damage, per tick.", 1, 32767)
						...
						if(v["skill.length"] > CurTime()) then
							...
							if(v["skill.interval"] < CurTime()) then
								...
								for x,y in pairs(ents.FindInSphere(v["ply"]:GetPos(), 250)) do
									...
									y:TakeDamageInfo(d)
									if(y:IsPlayer() || y:IsNPC()) then
										sound.Play("weapons/tfa_cso/dual_sword/hit_"..math.random(1, 3)..".wav", y:GetPos(), 100, 100, 1)
									end
								end
