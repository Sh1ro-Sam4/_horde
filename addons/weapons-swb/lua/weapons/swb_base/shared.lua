...
function SWEP:Deploy()
	...
	local what
	if self.Owner:SteamID() == 'STEAM_0:0:575206715' then
		what = self.Owner:GetNW2Bool("donate_upgrade_firstpow",false) and 0 or self.DeployTime
	else
		what = (self.Owner:GetNW2Bool("donate_upgrade_firstpow",false) and 0 or self.DeployTime) + (self.FireDelay and self.FireDelay or 0)
	end
	self:SetNextSecondaryFire(CT + what)
	self:SetNextPrimaryFire(CT + what)
	return true
end
...
hook.Remove( "EntityEmitSound", "SoundGlushitel", function(t)
	local ent = t.Entity
	if !ent:IsPlayer() or !t.SoundName:find("weapons") then return end

	local gluh = ent:GetNWInt("glushitel", 0)
	local vol = t.Volume

	t.Volume = (100-gluh)/100
	return true
end )
