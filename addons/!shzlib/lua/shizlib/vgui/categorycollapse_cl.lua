...
surface.CreateFont('shz-categorycollapse-tiny', {
    font = 'Roboto',
    size = 10,
})
...
function PANEL:Init()
	...
	self:SetFont('shz-categorycollapse-tiny')
end
...
function PANEL:UpdateColours(skin)
	if not self:GetParent():GetExpanded() then
		self:SetExpensiveShadow(0, Color(0,0,0,20))
...
function PANEL:Paint(w, h)
	if not self:GetParent():GetExpanded() then
		draw.SimpleText('u', 'Marlett', w, h * 0.5 - 2, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
end

derma.DefineControl('SHZCategoryHeader', 'Category Header', PANEL, 'XPButton')

--[[
	SHZCollapsibleCategory
]]
...
function PANEL:Init()
	self.Header = vgui.Create('SHZCategoryHeader', self)
	...
	self.ColorA = 25
	self.Color = CFG.theme.focus
end

function PANEL:Add(strName)
	local button = vgui.Create('SHZButton', self)
...
derma.DefineControl('SHZCollapsibleCategory', '', PANEL, 'Panel')