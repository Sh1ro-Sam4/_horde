...
function shizlib.circularMenu(opts)
	...
	if IsValid(shizlib.usePnl) then shizlib.usePnl:Remove() end
	...
	hook.Add('InputMouseApply', 'shizlib.use-menu', function(cmd, x, y, ang)

		if not IsValid(p) then hook.Remove('InputMouseApply', 'shizlib.use-menu') return end
		if p.active then
			if x ~= 0 or y ~= 0 then
	...
	hook.Add('CreateMove', 'shizlib.use-menu', function(cmd)

		if not IsValid(p) then
			hook.Remove('CreateMove', 'shizlib.use-menu')
		else
