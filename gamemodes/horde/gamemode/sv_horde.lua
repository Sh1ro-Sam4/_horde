...
function HORDE:SpawnEnemy(enemy, pos)
    ...
    spawned_enemy:SetMaxHealth(spawned_enemy:GetMaxHealth() * HORDE.difficulty_health_multiplier[HORDE.difficulty] * shizlib.horde.night_health_multiplier)
...
function HORDE:WaveStart()
    ...
    if HORDE.endless == 0 then
        ...
        HORDE.total_enemies_this_wave = HORDE.total_enemies_per_wave[HORDE.current_wave] *
            math.ceil(horde_players_count * (0.75 + difficulty_coefficient)) * shizlib.horde.enemy_count_multiplier
    else
